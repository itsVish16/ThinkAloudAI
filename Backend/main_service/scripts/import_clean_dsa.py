...
# Load .env to get settings, but we will override DATABASE_URL
load_dotenv(os.path.join(os.path.dirname(__file__), '..', '.env'))

async def run():
    db_url = os.environ.get("DATABASE_URL")
    
    # If running locally on EC2 (or inside docker), DATABASE_URL usually is correctly set in .env
    logger.info(f"Connecting to database at {db_url}...")
    engine = create_async_engine(db_url)
    LocalSession = sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
    
    json_path = os.path.join(os.path.dirname(__file__), '..', 'clean_dsa_questions.json')
    ...
    async with LocalSession() as session:
        ...
        logger.info(f"Imported {count} curated questions successfully.")
