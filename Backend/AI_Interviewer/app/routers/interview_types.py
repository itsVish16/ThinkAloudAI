...
async def get_token(
    ...
    try:
        if payload.interview_type == "dsa":
            response = await http_client.get(f"{settings.MAIN_SERVICE_URL}/dsa/questions", timeout=5.0)
            ...
            response = await http_client.get(url, params=query_params, timeout=5.0)
            if response.status_code == 200:
            ...
            response = await http_client.get(f"{settings.MAIN_SERVICE_URL}/aiml/questions?limit=2", timeout=5.0)
            if response.status_code == 200:
                data = response.json()
                if isinstance(data, list):
                    ai_selected_questions = data
